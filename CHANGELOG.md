...
### 性能
- 排序/搜索统一「内存 meta 排序 + 仅前 N 回查」：标题序（原「固有限制」声明
  被实测推翻）50k 约 1750→500ms（5k 实测 3.1×）；宽搜索词全量命中的回查悬崖
  3.7× 收口；搜索回查段补取消探针。fetcher 层路径判断收敛进 manager 统一分流。
...
### 安全
- 修复搜索分支缓存世代守卫回归：PERF-074 重写搜索链时 `decrypt_summary`
  掉落 `data_epoch` 透传，「搜索 worker 在飞 + 恢复重臂新世代」窗口下分类名
  缓存可被旧世代明文污染（SEC-043 漏点，一行修复 + 透传对称守护测试）。
- 导入时间戳校验改归一化：空格分隔/Z 后缀/截断时间等可解析变体统一归一为
  标准 ISO 形态后落值（拒绝式校验实测存在绕过变体）；备份恢复路径共用同一
  归一函数并补原地写回守护测试，消除恢复绕过导入约束的分叉。
...
### 修复与工程化
- 删除 `verify_raw` 零消费死函数（其描述的 PERF-067 就地验签场景已被回查架构
  取代，docstring 与现实颠倒误导读者）；SQL 排序映射键集补启动期自检断言。
- `custom_fields` 非 list 形态显式拒绝导入（原静默置空丢字段无感知）；
  侧边栏锁定守卫责任显式声明；分类批量通知参数对齐。
- 导出序列化拆分 `managers/exporters/` 策略包（与 `importers/` 对称，无
  dispatch 场景故不设策略协议——取舍已登记）；条目类型展示属性下沉 UI 资源层。
...
- 重构残留的过时 docstring（已删符号引用、与实现矛盾声明、失实历史表述、
  CLAUDE.md 架构描述跟随）批量修正；CHANGELOG 1.0.0 转正、审计索引漂移修正、
  零消费 re-export 清理、entry_batch_writer 协议化维持决策锚定注释补落地。
