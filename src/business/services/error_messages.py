...
def to_user_message(exc: BaseException, *, default: str = "操作失败，请重试。") -> str:
    ...
    if isinstance(exc, EntryIntegrityError):
        # EntryIntegrityError 是 ValueError 子类，须在下方 ValueError 分支前归一，
        # 避免 str(exc) 透传条目 crypto_id/字段名等技术细节。与 VaultIntegrityError 对齐。
        return "该条目数据完整性校验失败，可能已损坏。"
