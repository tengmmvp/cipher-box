...
def build_share_payload(
    ...
    *,
    ...
    for entry in entries:
        ...
        items.append(entry.to_dict(include_password=include_secrets))
    return {
        "format": SHARE_FORMAT,
...
def _render_decrypter_html() -> str:
    """渲染解密器 HTML。延迟导入 share_renderer，避免 resources 未就绪时本模块加载失败。

    renderer 与 WASM 资源在 share_renderer 模块就绪后才可用；延迟导入使本模块在
    资源/依赖缺失时仍可加载（仅 create_share_package 调用时才触发），便于分阶段实现与
    测试 monkeypatch 注入。
    """
...
def create_share_package(
    ...
    *,
    ...
    Returns:
        成功返回 ``(share_path, decrypter_path)``；取消返回 None。
    """
