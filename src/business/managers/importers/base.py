...
- ``_sanitize_totp_secret``：全部导入路径产出的 totp 统一有效（渲染层为安全边界，
  此处为数据卫生）。url scheme 清洗已上移至独立的 ``services/url_hygiene``，各策略类
  直接从那里导入。
...
def _sanitize_totp_secret(secret: str) -> str:
    ...
    与 url_hygiene.sanitize_url_scheme 同属导入数据卫生的一致性纵深防御。损坏的 totp 入库后虽
