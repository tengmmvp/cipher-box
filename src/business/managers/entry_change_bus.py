...
class EntryChangeBus:
    ...
    def __init__(self, cache: EntryCacheManager):
        self._cache = cache
        # 回调签名 (password_changed, metadata_changed, crypto_id)：订阅方据此判断是否需
        # 昂贵的缓存重算。metadata_changed 表示 title/username 等安全报告元数据是否变更，
        # 与 password_changed 共同决定 SecurityAnalyzer 是否失效（两者皆 False 的纯
        # 旁路变更如 is_favorite 切换可跳过，避免无谓整库重解密）。crypto_id 标识变更
        # 涉及的单条条目（PERF-021）：None=全量语义（增删/批量），订阅方可据此做单条
        # 增量更新而非整库重算——纯元数据编辑（改标题）与改单条密码均受益，避免每次
        # 保存触发整库密码解密 + HMAC 重算。
        self._callbacks: list[Callable[[bool, bool, str | None], None]] = []

    def register(self, callback: Callable[[bool, bool, str | None], None]) -> None:
    ...
    def notify(
        ...
        *,
        ...
        三个语义维度供订阅方（如 SecurityAnalyzer）决定是否跳过昂贵的缓存重算：
        ...
        - ``crypto_id``：变更涉及的单条条目（PERF-021）。None=全量语义（增删/批量/
          锁定/epoch 轮换），订阅方须整体失效重算；提供时随回调透传给订阅方做单条
          增量更新。
        ...
        for cb in self._callbacks:
            try:
                cb(password_changed, metadata_changed, crypto_id)
            except Exception:
