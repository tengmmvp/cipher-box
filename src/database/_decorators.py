...
``_db_operation`` / ``_db_write`` 独立成模块，避免 Repository 与 DatabaseManager 循环导入。
"""
...
class _DbOperationHost(Protocol):
    ...
    Concatenate 让 self 经 TypeVar bound 收敛到本协议，恢复静态检查。只读 property
    使 DatabaseManager 的实例属性与 Repository 的 property 转发均能满足。
    """
...
def _db_write(method: Callable[Concatenate[_HW, _P], _R]) -> Callable[Concatenate[_HW, _P], _R]:
    """数据库写操作装饰器：获取 RLock、校验连接、并执行写入前校验。

    相较 ``_db_operation`` 额外调用 ``self._guard_write()`` 阻止过期密钥会话继续
    写库，使写保护成为不可遗忘的默认（写方法十余处，靠手动调用易遗漏而静默绕过）。

    失败回滚契约：standalone 写依赖 sqlite3 默认 ``isolation_level`` 的隐式事务
    （首条 DML 自动开事务，末尾 ``_auto_commit`` 收尾）。方法抛异常时 ``_auto_commit``
    不执行，隐式事务悬挂而 ``_transaction_depth`` 仍为 0，此后显式 ``transaction()``
    的 ``BEGIN TRANSACTION`` 会抛 *"cannot start a transaction within a transaction"*。
    故 standalone 写失败须回滚隐式事务；显式事务内的嵌套写不在此回滚（由
    ``transaction()`` 统一回滚，避免双重 rollback 干扰外层）。
