...
class EntryRepository:
    ...
    def _entry_query_clauses(query: EntryQuery) -> tuple[str, list[Any]]:
        ...
        if query.after_id is not None:
            ...
            if query.tie_break_order:
                # 并列裁决键（PERF-087，仅搜索的排序下推分支经 tie_break_order
                # 开启，PERF-090 收窄）：排序列同值时按复合序尾部裁决（is_favorite
                # DESC, updated_at DESC，与下方默认复合序一致），使「行集序 ==
                # 内存稳定排序序（继承复合序）」的等价性在排序键并列 + limit 截断
                # 边界上成立（强度刻度 0-4 并列常见、批量导入 created_at 同刻）——
                # 不带裁决键时并列行序是引擎内序，截断入选集合会与「全量收集→
                # 稳定排序→截断」分叉。首键为 updated_at 时第三裁决键与首键同列
                # （并列行上恒同值，比较恒 no-op）冗余省去，语义不变。尾部子句
                # 列名硬编码，无注入面。SQL 直连路径（tie_break_order=False，
                # PERF-090）不追加：裁决键会使 ORDER BY 退化为 TEMP B-TREE
                # filesort（updated_at 序破坏 idx_entries_active_updated 前缀），
                # 而该路径没有需要对齐的内存排序路径，纯付成本无收益。
                if query.order_by == "updated_at":
                    sql += f" ORDER BY {column} {direction}, e.is_favorite DESC"
                else:
                    sql += f" ORDER BY {column} {direction}, e.is_favorite DESC, e.updated_at DESC"
            else:
    ...
    def get_entries(self, query: EntryQuery) -> list[RawEntry]:
        ...
        ``query.tie_break_order`` 为 True 时排序列同值按复合序并列裁决（见
        :meth:`_entry_query_clauses` 的 PERF-087 与 PERF-090 注释），默认 False 为纯单列
        序；``order_by=None``（默认）走复合序（is_favorite DESC, updated_at DESC）。
