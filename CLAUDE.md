...
### Crypto 层 (`src/crypto/`)
纯密码学原语，无数据库或 UI 依赖。`EncryptionEngine` 提供静态 AES-256-GCM 加解密方法。`MasterKeyManager` 通过 Argon2id（time=3 / 64MB / 并行=4）从主密码派生主材料，再经 HKDF-Expand 按域 info 派生主密钥、备份密钥与共享包密钥三个域（显式域分离，替代旧 salt 前缀）。密码验证不存储哈希，而是加密一段已知明文（验证令牌）来确认密码正确性。
...
编排 Crypto 和 Data 层；顶层 `composition.py`（CompositionRoot）的 `build_business_context` 集中组装 `BusinessContext`（config/vault/entry_mgr/security/import_export/backup），`CategoryManager`/`RestorePointManager` 提升为一等依赖显式创建并注入 EntryManager/BackupRestoreManager（保持替换间隙一致），并完成跨 manager 回调连线（锁定/变更→缓存失效 + 写事务提交→TOTP 缓存清空，SEC-063 统一失效 seam），供 MainWindow 注入，使 UI 不再自行 new 业务 manager。其余分为两个子包：
- `managers/`（有状态编排）：`VaultManager` 持有加密密钥和数据库连接，是安全边界的核心；`EntryManager` 透明地在写入时加密、读取时解密，聚焦写路径（加密、epoch 守卫事务、变更通知与缓存失效差分）；查询读族——因加密字段无法在数据库层面搜索，搜索经窄投影（SearchRow）拉取解密后在内存过滤、命中行按 id 回查完整行验签（PERF-074），详情读路径经 `get_entry_with_epoch` 返回 `EntryRead`（entry+data_epoch+data_version 三元组）供 detail_panel/TOTP 预热携带世代守卫（SEC-054/SEC-063）——下沉 `services/entry_queries` 的 `EntryQueryService`（MAINT-116），EntryManager 公开查询方法保持薄委托；配套 `EntryCacheManager` 提供摘要/分类名/标签/TOTP secret/搜索投影行集多级缓存，经 `EntryChangeBus` 统一「变更→缓存失效→回调」管线，并经 property 暴露 `categories`(`CategoryManager`)/`totp`(`TotpService`)/`password_history`(`PasswordHistoryService`) 子服务（SRP 拆分自原 EntryManager）；`ImportExportManager` 负责导入/导出编排（格式解析拆分至 `managers/importers/` 策略类包：JSON/CSV/KeePass/Bitwarden 各一策略类；导出序列化拆分至 `managers/exporters/` 策略函数包），导入写入经 `@_validate_import_input` 校验路径后，在 `_import_entries` 的 epoch 守卫事务内原子完成（加密移出 db_lock，MAINT-004）；`BackupRestoreManager` 实现可移植的二进制加密备份格式（持锁核心：创建/恢复/可移植数据采集），使用独立的备份密码派生密钥（经 HKDF 与主密钥域分离），支持跨主密码恢复，经 `restore_points` property 暴露 `RestorePointManager`；生命周期流程（初始化/解锁/锁定/改密/关闭）拆至 `VaultLifecycleOrchestrator`，`VaultManager` 收窄为密钥/db/写守卫核心（备份目录收集与 purge 清理下沉至 `services/backup/purge` 纯函数，经 `config` property 暴露 ConfigManager 供其取 data_dir/backup_directory），生命周期委托经 `LifecyclePort` 协议（不再 TYPE_CHECKING 依赖具体编排器类型），使调用方（app/login/dialog/test）无需感知编排层
- `services/`（无数据库事务编排的协作模块，可有缓存/派生状态），按子域分组：
  - 字段与校验：`crypto_utils` 统一字段加解密入口与 `SENSITIVE_ENCRYPTED_FIELDS` 单一事实源（搜索谓词/视图构造/portable dict 解密已按职责域拆出至 entry_search_match/entry_view_decryption/backup.collector，MAINT-097）；`entry_validation` 明文条目校验；`entry_search_match` 列表搜索与标签过滤的匹配谓词纯函数（UI 过滤与查询服务搜索热路径共用）；`entry_sorting` 条目排序键提取器纯函数（内存排序路径的单一事实源——唯一生产消费方 entry_queries，QL-074 删除 UI sort_entries 后；MAINT-104 迁出 managers）；`url_hygiene` URL scheme 白名单与表格公式注入前缀转义纯函数（导入/导出/共享包共用）；`entry_type_schema` 5 种条目类型的字段 schema 注册表单一事实源（供 entry_dialog/custom_fields_renderer/import_export 消费）；`password_service`/`card_validation` 作为切断 UI→Crypto 跨层依赖的业务门面
  - 条目子域：`totp_service`/`password_history_service` 条目子域服务；`entry_batch_writer` 导入批量写入加密/落库编排（从 EntryManager 下沉，覆盖路径经 `entry_repository.update_overwrite_batch` 单事务批量写入 + 批量密码历史；兼进度契约单一事实源——`phase_progress`/`should_report_progress` 供导入/导出/恢复各 worker 消费（MAINT-099）、`write_chunks` 分块写入共享原语供覆盖写入/恢复重建复用（MAINT-106）、`ProgressSegment`/`segment_progress`/`validate_progress_segments` 段表件供三组加权刻度共用（MAINT-112））；`entry_view_decryption` 视图构造与解密族（`copy_entry_fields`/`build_entry_summary` RawEntry→Entry 视图构造原语 + `EntryViewDecryptor` detail/export/summary 三视图与严格/容错字段解密的纯变换，从 EntryManager 下沉，EntryManager 公开方法保持薄委托）；`entry_queries` 查询读服务（`EntryQueryService`：列表摘要/详情/近期更新/导入去重/导出读取的锁内外编排 + `projection_cache_key` 投影缓存键构造，从 EntryManager 下沉，MAINT-116；vault 经 TYPE_CHECKING 具体类锚定 ARCH-039，cache 经 `QueryCacheProtocol` 协议注入，view_decryptor 注入宿主共享实例）
  - 备份无状态子包 `backup/`（`BackupRestoreManager` 下沉的纯函数模块，SRP 拆分）：`header_codec`（二进制头编解码/检视）、`validator`（恢复前载荷校验）、`paths`（文件命名约定）、`payload`（Portable 系列 TypedDict 与 PreparedBackup 载荷类型）、`collector`（可移植数据采集+载荷大小校验，含 `decrypt_entry_to_portable_dict` 整条解密）、`rebuilder`（恢复重建纯变换：载荷→加密行逐表回写，条目写入经 `entry_batch_writer.write_chunks` 分块上报，MAINT-106）、`purge`（备份文件清理纯函数：目录收集 + snapshot/恢复点 purge，下沉自 VaultManager）、`auto_backup_policy`（自动备份间隔判定/retention 清理纯策略）
  - 限时加密共享子包 `share/`（`.cboxshare`/`decrypt.html`）：`share/header_codec`/`share/package`/`share/paths`/`share/renderer` + `share/resources/`（打包的解密器 HTML/JS 资源，file:// 下用纯 JS 库 hash-wasm+asmcrypto）
  - 完整性与重加密：`key_manager` 持有/清零主密钥与快照密钥；`metadata_signer` 提供条目元数据、分类元数据与 `vault_meta` 的 HMAC 完整性签名；`re_encryption` 编排改密时的全量重加密；`security_analyzer` 弱密码/重复/过期分析（持 TTL 缓存，失效粒度按 `EntryChangeBus` 的 password_changed/metadata_changed 维度精确控制，纯旁路变更如收藏切换/分类调整不触发整库重算）
  - 配置与文案：`vault_meta_store`（`vault_meta` 键值持久化，供生命周期流程读写 salt/verify/kdf 参数/snapshot_key/mac 等）、`vault_meta_keys`（`vault_meta` 键名常量单一事实源，供 vault_lifecycle/metadata_signer/backup_restore 派生）、`error_messages`（领域异常→用户文案翻译，`to_user_message`）、`database_bootstrap`（初始数据库引导/默认分类注入）、`rate_limiter`（登录/改密等敏感操作的递增退避限流，状态文件经哨兵+签名见证抵抗删除绕过）
