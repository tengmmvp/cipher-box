...
from src.models import MAX_PASSWORD_HISTORY, Category, RawEntry
...
def test_get_entries_by_ids_preserves_request_order_against_db_order(db):
    """按请求顺序返回，而非 DB 的 id ASC（SQL IN 不保序，须 Python 重排）。

    反向输入（大 id 在前）能区分「真重排」与「巧合的 DB ASC」——后者会返回 [id1, id2]。
    """
    id1 = db.add_entry(_make_entry(crypto_id="c1", title="A"))
    id2 = db.add_entry(_make_entry(crypto_id="c2", title="B"))
    fetched = db.get_entries_by_ids([id2, id1])
    assert [e.id for e in fetched] == [id2, id1]


def test_update_entry_raises_for_missing_id(db):
    """update_entry 对不存在的 id 抛 DatabaseError（显式失败而非静默 UPDATE 0 行）。"""
    entry = _make_entry(id=999999, crypto_id="missing", title="X")
    with pytest.raises(DatabaseError, match="不存在"):
        db.update_entry(entry)


def test_update_category_raises_for_missing_id(db):
    """update_category 对不存在的 id 抛 DatabaseError（与 update_entry 对称）。"""
    category = Category(id=999999, name="Missing")
    with pytest.raises(DatabaseError, match="不存在"):
...
def test_update_entries_batch_happy_path(secure_db):
    ...
    """
    entries = [_make_encrypted_entry(crypto_id=f"upd-{i}", title=f"原标题{i}") for i in range(3)]
    id_map = secure_db.add_entries_batch(entries)
...
def test_update_password_history_batch_happy_path(secure_db):
    ...
    secure_db.add_password_history(
        entry_id,
        _enc("old-pwd-1", crypto_id, "password"),
        "2026-01-01T00:00:00Z",
    )
    secure_db.add_password_history(
        entry_id,
        _enc("old-pwd-2", crypto_id, "password"),
