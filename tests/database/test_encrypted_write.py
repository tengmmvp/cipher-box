...
def secure_db(tmp_path):
    ...
    db.open()
    db.init_tables()
    # 非 test_mode 构造，_enforce_encrypted_fields 默认即为 True（生产态启用加密断言）。
...
def _make_encrypted_entry(**overrides):
    """构建一个带有 cb2: 前缀密文的 RawEntry，模拟已加密状态。

    注意：此函数硬编码了 RawEntry 的全部字段。RawEntry 新增字段时，
    此处的构造参数须同步更新，否则构造时会因缺少默认值而报错。
    """
    entry = RawEntry(
        id=None,
