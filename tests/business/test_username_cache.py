"""测试 EntryManager 用户名缓存的 epoch 失效行为。"""
...
class TestUsernameCacheEpochInvalidation:
    ...
    def test_same_epoch_keeps_cache(self):
        ...
        assert mgr._username_cache == {'id1': 'user1'}
    ...
    def test_invalidate_caches_clears_all(self):
        ...
        mgr = EntryManager(vault)
        mgr._username_cache = {'id1': 'user1'}
        mgr._username_decrypt_failed = {'id2'}

        mgr.invalidate_caches()

        assert len(mgr._username_cache) == 0
        assert len(mgr._username_decrypt_failed) == 0
