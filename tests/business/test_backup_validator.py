...
class TestValidateRestoreData:
    ...
    def test_too_many_categories_rejected(self):
        data = _valid_restore_data()
        data["categories"] = [_valid_category(i) for i in range(10_001)]
    ...
    def test_too_many_history_rejected(self):
        ...
        data["password_history"] = [_valid_history(1) for _ in range(MAX_HISTORY_PER_ENTRY + 2)]
    ...
    def test_history_limit_scales_with_entries(self):
        ...
        data["password_history"] = [
            _valid_history(1 if i % 2 else 2) for i in range(MAX_HISTORY_PER_ENTRY * 2 + 1)
        ]
