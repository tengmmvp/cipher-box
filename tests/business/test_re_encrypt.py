"""_re_encrypt_all 边界条件测试，覆盖空字段、custom_fields 类型、密码历史、事务回滚与损坏中止。

通过 change_master_password 触发全量重加密，验证空字段保持空、
已删除条目、密码历史、自定义字段在改密后均可正确解密，
并验证写入失败或密文损坏时事务回滚/中止保障数据完整性，
以及改密后分类以新域密钥重签。
"""
...
class TestReEncryptEdgeCases:
    ...
    def test_re_encrypt_aborts_on_corrupt_entry_preserves_data(self):
        ...
        corrupt_id = self._entry_mgr.add_entry(
            Entry(
                title="损坏条目",
                username="bad_user",
                password="temp_pass",
