...
建库经 make_vault_env 工厂统一装配与回收（teardown 幂等 close）。
"""
...
class TestRestorePointCleanup:
    ...
    def test_purge_removes_restore_points(self, make_vault_env):
    ...
    def test_purge_keeps_regular_snapshots(self, make_vault_env):
    ...
    def test_purge_missing_directory_is_noop(self, make_vault_env):
        """backups 目录不存在时安全无操作，返回空失败列表。"""
        vault = make_vault_env().vault
        failed = purge_restore_points(vault.config)
        assert failed == []

    def test_clear_all_removes_restore_points_and_returns_count(self, make_vault_env):
    ...
    def test_clear_all_preserves_snapshots(self, make_vault_env):
    ...
    def test_clear_all_returns_partial_count_on_purge_failure(self, make_vault_env, monkeypatch):
...
def test_change_master_password_reports_purge_failure(make_vault_env, monkeypatch):
