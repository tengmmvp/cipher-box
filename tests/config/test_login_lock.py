...
验证锁定状态使用 time.time() 墙上时钟而非 time.monotonic()，确保跨进程重启后
仍能正确判断锁定是否过期。

当前测试仅验证 JSON 文件读写与时间比较逻辑，未导入或测试 LoginWindow 的实际
锁定管理代码。待 LoginWindow 的锁定状态管理提取为独立可测试函数后，应替换为
针对实际业务逻辑的测试。
"""
