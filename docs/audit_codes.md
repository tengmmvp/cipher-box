...
- **编号分配**：新编号 = 该维度当前最大已用号 +1；历史断档（如 ARCH-012→021、
  ...
  `ARCH-063 / MAINT-125 / PERF-104 / QL-083 / SEC-076`（由
...
| `ARCH-005` | `ARCH-005` | 18 | 比对内存与库内 epoch 不一致时抛出（ARCH-005）：中止读路径以防用旧密钥解密新密文 |
...
| `ARCH-060` | — | 1 | entry_cache 结构观察登记：六域缓存+双域失效版本+条目粒度水位共存于单一 EntryCacheManager（单锁统一失效矩阵，域内聚有据）；后续按域拆分的前置条件是先论证锁拆分顺序。 |
| `ARCH-061` | — | 1 | 损坏库启动的用户可见错误反馈：LoginWindow 构造（ensure_db_open 触发库结构校验）原无兜底、异常直达 sys.excepthook（打包无控制台下无声退出）；_show_login 构造段包 try/except 对齐主窗口失败分支，QMessageBox 翻译文案后干净退出。 |
| `ARCH-062` | — | 3 | 锁定后拒绝重登的退出路径清理对称化：登录窗 Rejected 分支 quit() 不触发 closeEvent、退出清理与窗口状态持久化均未执行；main_window 新增公共 perform_exit_cleanup（与 _quit_app 同序），app 侧 quit 前调用。 |
...
| `MAINT-102` | — | 0 | 删除零调用死代码 EntryCacheManager.search_lower_no_check（docstring 自述被 cached_search_metadata_full 取代，全库零调用，能力为 full 后 4 字段子集）；cached_search_metadata/cached_search_metadata_full docstring 中对它的过时引用同步修正。（处数归零：唯一 src 引用随 MAINT-119 删除 cached_search_metadata_full 消失，决策已终结。） |
...
| `MAINT-118` | — | 1 | 生产死代码删除族：EntryCacheManager.cached_search_metadata_full（消费方已迁 batch 会话，src 零调用）与 EntryManager.decrypt_entry_for_export 死委托删除，测试改经新入口。 |
| `MAINT-119` | — | 5 | 摘要取值三胞胎收敛：cached_search_metadata/search_metadata_for_analysis/已删的 _full 三公开变体收敛为单一 cached_search_metadata（docstring 声明「单条用本方法 / 批量用会话」规则），for_analysis 随 PERF-098 消费方迁移删除、协议声明同步换名；_SearchMetadataBatch 增 pending 只读 property 消除跨类私有访问。 |
| `MAINT-120` | — | 7 | 守卫读骨架提取：entry_queries 四处重复的「失效重臂 → 锁内拉行集+同刻快照 → 锁外解密 → epoch 失配归一返回空」骨架收敛为 _guarded_snapshot 私有辅助；export 有意不走（失配上抛语义不同）。 |
| `MAINT-121` | — | 1 | 缓存失效命名统一复数：SecurityAnalyzer.invalidate_cache → invalidate_caches，与 EntryManager/CategoryManager 对齐，调用方同步。 |
| `MAINT-122` | — | 1 | EntryManager 类 docstring 标注查询薄委托（MAINT-021×1+MAINT-116×5）的过渡保留与退役路径，防宽接口无计划长期固化。 |
| `MAINT-123` | — | 5 | UI 与配置域私有面/常量收编族：WorkerBackedDialog 增 is_irreversible_running() 谓词消除 _worker 私有直访；DIALOG_CONTENT_MARGINS 常量收编两处逐字重复边距；config.THEME_DARK 常量消除 "dark" 字面量双源（派生恒等测试守护，对齐 ARCH-035 先例）。 |
| `MAINT-124` | — | 1 | _restrict_windows_acl 去下划线公开为 restrict_windows_acl：私有名构成的跨模块契约与可见性不符，对齐 PERF-089 先例；注入契约语义不变。 |
...
| `PERF-086` | — | 31 | 搜索暖缓存两件套：a) EntryCacheManager 投影行集缓存 search_projection_rows——键=(deleted_only, category_id, favorite_only, order_by, order_desc)（复合序规范化为 (None,True) 防同义键占槽；有序行集因消费方依赖行序不可与无序混存），容量 4 键 LRU（单键最坏 50k 行 SearchRow ~20MB，全密文+明文定位/排序列，无明文驻留顾虑），以主域 _invalidate_version 失效（增删改/导入/恢复/锁定/改密全路径经 apply_change/invalidate_all 推进；QL-070 分域后单条 TOTP 失效不再击穿本缓存）；出口浅拷贝隔离（命中/回填两路径均返回 list 新容器，调用方变异不污染缓存，50k 行引用拷贝 ~0.4ms 可忽略）；b) search_metadata_batch 批量摘要会话：循环外一次持锁快照命中集（dict 拷贝）+ 锁外解密 + 一次持锁整批 epoch+version 守卫回写（try/finally 包 yield，with 体抛异常退出同样回写 pending），替代逐行 RLock+move_to_end（50k 行 ~78ms）；批量命中不推进 LRU recency（仅超条目数上限的理论场景产生淘汰、无正确性影响）。get_entry_summaries 的 invalidate_if_epoch_changed 前移至读块前（原在块后，首次调用的 epoch 重臂会把刚回填的投影缓存清掉）。实测 5k 暖态：search+limit 44.3→5.1ms、全量搜索 349.8→231.5ms。 |
...
| `PERF-092` | — | 4 | full_analysis 全程持 vault_write_lock（50k 库 ~5s）的取锁阻塞收敛——改密入口对称取消 + 持锁取舍显式化：a) change_master_password 取写锁前 request_cancel（与 lock()/close() 同款），在飞分析的阻塞从「分析全程」缩短到「取消检查点间隔（64 条）+ unwind」，finally 兜底清事件防认证失败路径残留置位误取消后续分析（_re_encrypt_all 开头 clear 自行复位）；b) 评估「锁内快照 entries+key、锁外分类」的锁拆分后**维持全程持锁**并在 full_analysis 注释声明取舍——正确性守卫（_cached_analysis 写回的 epoch+失效世代、_make_summary 的 SEC-043）确已支撑锁外执行，但全程持锁承载的是安全不变量：lock()/close() 清零密钥并 gc.collect 须等 worker 释放全部密钥/明文引用后才真正可回收，拆分会重开「后台 Worker 超时后仍持密钥」窗口（持锁设计要关闭的）；取消机制已把所有时敏竞争方（lock/close/改密）的等待封顶，唯一不取消方 password_history_service.decrypt 为 UI 短操作，接受其余时长阻塞。 |
...
| `PERF-096` | — | 1 | 投影缓存容量 4→8：8 种排序×3 视图+近期更新可产出 >4 常用键，频繁切换排序触发 LRU 颠簸重拉全量行集；单键 ~20MB/50k，容量 8 内存可控。 |
| `PERF-097` | — | 1 | search_metadata_batch 会话快照拷贝登记保留：拷贝是循环零锁读前提（快照与世代采样配对支撑整批回写守卫），成本仅占摊销掉的逐行锁往返 ~2%，世代号+追加式替代需重构失效矩阵不值；顺带空缓存跳过拷贝。 |
| `PERF-098` | — | 3 | 摘要行循环接批量会话：_summaries_from_raw_rows 与 get_recent_summaries 每行 4 次 RLock 改经 search_metadata_batch 会话摊销为进出各一次；decrypt_summary 的 skip_epoch_check 参数随之全量退役。review 补正：行集 ≤ _SUMMARY_BATCH_MIN_ROWS(128) 分流单条入口（小 N 免付全量快照拷贝），batch 会话增 failed 集即时回写通道（解密失败即刻持锁按 epoch+version 守卫写入，冷缓存首轮 integrity_error 不漏标，与单条路径语义统一）。 |
| `PERF-099` | — | 3 | toggle_favorite 存在性前置改 EXISTS 轻查询：原 get_entry 拉 20 列宽行且事务内复读；entry_repository 新增 entry_exists（SELECT 1），经 db_manager 委托与 EntryStore 协议接线；错误语义不变。 |
| `PERF-100` | — | 1 | 搜索匹配谓词保留 4 元组传参登记：改传 SearchMetadata 会把视图解密域类型耦合进 UI 共用纯函数，净收益趋零。 |
| `PERF-101` | — | 7 | clear_category_signatures 批量签名消除逐条物化：原逐条 _row_to_entry+两次 replace+逐属性 HMAC（O(N) 持锁循环，50k 全归一类 ≈1-2s UI 冻结）；MetadataSigner 新增 sign_entry_from_row 行字典直签（_sign_payload 私有原语与 sign 共用锁检查+HMAC），经 EntrySigner 注入缝传递、协议放宽为 RawEntry | Mapping 单点回退（防 Data→Business 反向依赖），两路径载荷收敛单一渲染器 _payload_from_row 保证字节一致；_payload 键集提升 _PAYLOAD_ROW_ATTRS 注册表并由测试钉住与 _ENTRY_COLUMN_GETTERS 的一致性（防新列脱离 MAC 保护）。 |
| `PERF-102` | — | 1 | 解锁后全量分析 CPU 峰值取舍登记：指纹绑定主密钥、跨锁定保留缓存不可行（安全要求），每 64 条取消探针已封顶 lock() 等待——设计取舍维持现状（PERF-092 同域延伸）。 |
| `PERF-103` | — | 1 | 导出路径 fetchall 双份驻留取舍登记：导出为低频显式操作且备份侧有载荷校验，流式分批改造涉及进度契约与锁语义重排不成比例，维持现状。 |
...
| `QL-019` | — | 6 | ``prepare_password_update`` 的 ``hmac.compare_digest`` 两边 ``encode("utf-8")``：str 版仅支持 ASCII，非 ASCII 密码条目编辑/覆盖导入抛 TypeError 永不可保存。 |
...
| `QL-079` | — | 7 | 异常捕获与类型纪律漏网收编族：vault_lifecycle 内联密码比较改 PasswordService.passwords_match（SEC-031 漏网）；四处 secure_checkpoint 降级 except Exception 收窄为 (DatabaseError, OSError)（契约实际抛此二元组，OSError 有测试 mock 守护）并经 services/db_checkpoint.safe_checkpoint 共享助手收敛（empty_trash 漏写 OSError 的 review 补正后四处统一）；三处 type: ignore 补行内理由；rate_limiter 异常元组精简去冗余。 |
| `QL-080` | — | 1 | 索引幂等补建异常归一：CREATE UNIQUE INDEX 遇既有库重复 crypto_id 时裸 sqlite3.IntegrityError 逃出 init_tables，补建循环包 except sqlite3.IntegrityError 转 SchemaError（含索引名的中文文案）；OperationalError 等瞬时/环境失败按原样传播不误诊为结构损坏。 |
| `QL-081` | — | 1 | secure_compare 纪律文本显式豁免 bytes 形态比较（compare_digest 接受任意 bytes，无 str 的 TypeError 风险），防后来者误判违规。 |
| `QL-082` | — | 4 | 时间戳+uuid 文件名构造共享：backup/paths 与 share/paths 双站点重复收敛 format.timestamped_suffix（时钟可注入），格式不变。 |
...
| `SEC-011` | `SEC-011` | 0 | # SEC-011：id 反查须在 _auto_commit() 之前完成——插入与反查在同一隐式事务内（处数归零：SEC-073 预分配 id 单段式插入删除了 crypto_id 反查，决策已终结。） |
...
| `SEC-031` | — | 6 | 确认密码常量时间比较统一 PasswordService.passwords_match 门面（utf-8 encode），四处调用点收编，防 QL-019 同型漏 encode 复发。 |
...
| `SEC-049` | — | 2 | decrypt_entry_for_export 三层补 data_epoch 世代守卫透传（ViewDecryptor→EntryManager 薄委托→get_entries_for_export 锁内快照）：导出 worker 在飞 + 恢复提交重臂新世代交错下，旧密钥解出的分类名经缓存回写植入新世代——SEC-043 全读路径接入的 export 链漏点（SEC-040 同级防御纵深）。 |
...
| `SEC-073` | — | 8 | 条目签名载荷纳入 entry.id：堵整行复制改 id 的克隆条目绕过验签（与 SEC-004 同威胁边界的纵深补齐）；插入路径重构为预分配 id 单段式——锁内 _next_entry_id()（max(seq, MAX(id))+1，AUTOINCREMENT 显式赋值正确推进 sqlite_sequence，实测三点验证含失败回滚还原）后 _signed_insert_row() 单一落点携带最终 mac 一次 INSERT（未来新插入路径无法跳过重签）；SEC-011 的 crypto_id 反查随之删除。 |
| `SEC-074` | — | 1 | verify 令牌比较收编 constant_time_mac_equals：非 ASCII 解密明文不再抛 TypeError 逃出 except ValueError（异常语义漂移，纵深对齐 SEC-071 全站点单一入口）。 |
| `SEC-075` | — | 2 | 限流读侧 remaining_seconds 封顶：SEC-064 降级会话下伪造超大值致近乎永久锁定；读侧 min() 封顶为合法最高档 2 倍，把伪造影响压缩到与合法最高阶梯同量级，不改变权衡本体。 |
