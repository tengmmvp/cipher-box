...
| 本地写权限者篡改/删除限流状态归零计数 | 状态文件 HMAC-SHA256 签名（SEC-029），改写/验签失败按最高阶梯保守锁定并自愈重写；哨兵文件区分「首次使用」与「状态被删除」，删除不绕过退避；无签名降级时状态完全不落盘（SEC-042），仅内存限流；签名密钥降级会话（DPAPI/写盘失败，SEC-055/065）磁盘既有状态文件无法验签，内容按面值采信并记 WARNING（SEC-064：确定性误伤诚实降级会话 > 低概率理论窗口，会话内 3 次→10s 退避仍生效）——已知残余面：文件写权限者可把 config.key 置为不可写使此后每个会话走该降级（限流仅内存、跨重启清零，配合每次启动前篡改状态文件可绕过跨会话退避），但该能力下攻击者可直接复制 vault.db 离线攻击（严格更优），实际增量有限，属可接受残余 |
| Windows DPAPI 持续故障 | protect 失败不落盘（SEC-055），内存密钥仅本会话有效并记 CRITICAL；`session_only` 标记使限流器拒绝以临时密钥签名落盘（SEC-057），避免下次启动签名失配误锁 10 分钟 |
| keyring 不可用（headless Linux / CI） | 回退明文 0600 `config.key` 并记 ERROR 使降级可见（SEC-003）；keyring 恢复后一次性回迁系统密钥链并清理明文文件，保护对「keyring 曾故障」的安装重新生效 |
